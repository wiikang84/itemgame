...
### 사다리 "NEON LADDER" (v1.2)
- **구조**: 2선 사다리, 3줄 or 4줄 가로선 (50/50 랜덤)
- **베팅 2종 (v1.2 단순화)**:
  - 홀짝 (1.95배): 홀 / 짝
  - 조합 (3.8배): 홀3 / 짝3 / 홀4 / 짝4
- **Canvas 기반 네온 비주얼**: 시안 사다리선 + 퍼플 가로선 + 귀여운 애벌레 캐릭터
- **극적 슬로모션 연출 (~20초)**: 카운트다운(2.7초) → 가로선 순차 공개(3.2초) → 출발점 깜빡(2초) → 애벌레 하강(~12초)
- **긴장감 시스템**: 가로선 교차 후 0.5초 정지, 마지막 하강 전 1.2초 서스펜스, 최종 하강 3초 QuintEasing 슬로모션
